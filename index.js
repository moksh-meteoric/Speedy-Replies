...
function scheduleScorecard() {
  function msUntil10pmIST() {
    ...
    // Build target: today at 22:00 IST
    const targetIST = new Date(nowIST);
    targetIST.setHours(22, 0, 0, 0);
    // If already past 10pm IST today, schedule for tomorrow
    if (targetIST <= nowIST) targetIST.setDate(targetIST.getDate() + 1);
    // Convert back to ms delay from real now
    return targetIST.getTime() - istOffset - nowUTC;
  }
  function loop() {
